...
function do_load_git( repo_name ) {
    ...
    d3.json("/git_repositories/" + repo_name + ".json", function(error, json) {
...
function load_git() {
    if($('#git_summary_controller').val() == 'true' ){
        JSON.parse($('#git_repositories').val()).forEach(function(repository) {
            do_load_git(repository);
        });
    }
};
